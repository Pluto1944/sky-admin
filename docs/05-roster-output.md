...
**表头**：`ARRANGEMENT_OUTPUT_HEADERS`（14 列，定义于 `config.py`）
...
**原因**：避免 Part1-3 的 14 列表头在 Part4 区域产生多余的空白列，保持表格整洁。
...
| `ARRANGEMENT_OUTPUT_HEADERS` | `config.py` | Part1-3 的 14 列输出表头定义 |
...
| `config.py` | `ARRANGEMENT_OUTPUT_HEADERS`（14 列，含 `team_name`） |
