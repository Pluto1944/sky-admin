...
## v3.1 — 名单稳定重排与升降级保护（2026-09）

- 保持现有升降级候选、星数门槛和每对最多 2 人规则不变。
- 删除本月缺失成员时采用稳定向上补位，降级成员不得被补回原队伍；不锁定队内具体位置。
- 战营新增（位置 30）和普通营实战新增（实战区末尾）规则不变；新增导致升级成员跌破目标队伍时执行局部兜底。
- 白名单最后执行但业务优先级最高；尽量同时满足升降级边界，无法满足时以白名单目标为准并输出告警。
- 实战与壳子继续共用线性名单：实战不足时壳子向前填充，实战超容量时进入壳子队伍。

## v3.2 — 移除旧升降级模块（2026-09）

- 删除历史独立升降级模块 `modules/cwl_registration/promotion.py`。
- 删除仅覆盖该旧入口的 `tests/cwl_registration/test_promotion.py`。
- 当前升降级统一由 `baseline_rebuilder.py` 阶段 2 执行，保护逻辑由 `team_builder.py` 复核。
