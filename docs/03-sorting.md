...
**回答（历史设计）**：(A) 按综合分（rank_score）插入到合适位置。该方案已被 v2.7 废弃；当前实现统一追加到实战区末尾，`new_normal_insert_start` 仅保留兼容配置。
...
**回答（历史设计）**：(A) 只把名单7 的人逐个二分插入到编号45之后的现有列表中。该方案已被 v2.7 废弃；当前名单7 统一追加到实战区末尾。
...
v3.1 延续 v3.0 的核心思路：以**上月实战名单为锚点**做基准重建，而非完全重排后微调；并在所有后续名单调整中增加升降级边界保护。
...
从 `final_list` 中删除名单4 成员。不使用对外占位符，其他成员保持相对顺序向上补位。

删除补位时只保留降级边界：降级成员不能被补回原来的更强队伍，但不锁定其在目标队伍内的具体名次。升级成员不做保护，可以随整体继续向上。删除人员记录到 `removed_list`（供 Part3 展示）。

#### 阶段 4：插入战营实战新增（`insert_combat_new`）

在 `final_list` 编号 `NEW_COMBAT_INSERT_START`（默认30，即第3队开头）处连续插入名单6 所有成员，标记 `movement = "新"`。插入后其他成员保持相对顺序向下平移；固定编号落在实战队伍范围内时，战营新人不会被兜底修复提前拉入该编号之前的队伍；若升级成员被推到升级目标以下，则执行稳定局部修复，保证升级边界。

> 原因：前两支高等级队伍不放新人，新人从第3队起插入。

#### 阶段 5：插入普通营实战新增（`insert_normal_new`）

名单7 全部追加到实战区末尾（壳子区之前）。当前实现为统一追加，不再按综合分二分插入。新增后的稳定重排同样保护升级边界；降级成员不保护，可以继续向更弱队伍移动。

> 原因：新人放后面，老人放前面。

#### 阶段 6：追加壳子名单（`append_shell`）

名单3 按综合分降序追加到 `final_list` 末尾。最终 `final_list = [实战区] + [壳子区]`。配置中的队伍顺序同样保持“先实战、后壳子”，只允许调整两类队伍的数量，不允许实战与壳子队伍交错。
...
由于实战和壳子共用同一个线性池，实战人数不足时，壳子成员会向前填充实战队伍；实战人数超过实战队伍总容量时，实战成员会继续进入壳子队伍。该行为保持现有规则不变。

队伍填充后会再次校验升降级边界，防止容量切分、壳子补位或白名单级联造成升级成员跌破升级目标，或降级成员回到原队伍。

总人数超过配置容量时，最后一支配置队伍继续按旧规则吸收超员，并在 stderr 输出容量告警；边界保护无法同时满足时另行输出冲突告警。
...
白名单是最高优先级。白名单插入完成后仍会尝试维持升降级边界：普通成员优先被挤压和顺延；如果白名单目标与升降级边界无法同时满足，以白名单目标为准并打印告警日志。展示字段保持不变，仍按现有规则标记 `↑升级` / `↓降级`、`新` 等，不新增内部补位标签。
...
v2.x 的排序流程是：`sort_accounts()` 完全重排 → `fill_teams()` 贪心填充 → `apply_promotion_relegation()` 局部微调。这里的 `promotion.py` 仅作为历史设计记录，文件已在 v3.2 中移除。
...
| 升降级 | `apply_promotion_relegation` 局部微调（历史） | 阶段2 在上月分组上配对交换 |
| 黑名单 | 无 | 阶段0 前置过滤 |
| 白名单 | 无 | 阶段8 填队后强制插入 |
