"""报名模块 · 功能②：根据报名快照 + 账号得分生成联赛名单（v3.1 基准重建方案）。

v3.1 数据流（基准升降级与稳定重排保护）：
...
4. 【基准重建】build_final_list：以上月名单为锚点，构建最终线性名单
   ...
   - 阶段3：删除缺失人员并稳定向上补位
   - 阶段4：插入战营新增（编号30），并兜底保护升级边界
   - 阶段5：插入普通营新增（实战区末尾），并兜底保护升级边界
   - 阶段6：追加壳子名单
...
class LeagueArranger:
    ...
    def arrange(
        ...
        v3.1 基准重建流程（含稳定重排保护）：
          ...
          阶段 3：删除实战缺失人员并稳定向上补位
          阶段 4：插入战营实战新增（编号30），保护升级边界
          阶段 5：插入普通营实战新增（实战区末尾），保护升级边界
          阶段 6：追加壳子名单
          阶段 7：贪心填充队伍
          阶段 8：白名单处理（最高优先级，冲突告警）
